...
import MonteCarloMini.SearchParallel.Directions;
...
public class TerrainArea {
  ...
  //direction for searchparallel
        Directions next_steps( int x, int y) {
		Directions climb_direction =Directions.STAY_HERE;
		int height;
		int local_min= get_height(x, y);
		if ( x > 0 ) {
			height=get_height(x-1, y);
			if (height<local_min) {
				local_min=height;
				climb_direction = Directions.LEFT;
			}
		}
		if ( x < (rows-1) ) {
			height=get_height(x+1, y);
			if (height<local_min) {
				local_min=height;
				climb_direction = Directions.RIGHT;
			}
		}
		if ( y > 0 ) {
			height=get_height(x, y-1);
		...
		if ( y < (columns-1) ) {
			...
			if (height<local_min) {
				local_min=height;
				climb_direction = Directions.DOWN;
			}
		}
		return climb_direction;
	}
